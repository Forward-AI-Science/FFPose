"""LiteHRNet pose estimator: LiteHRNet backbone + HeatmapHead + MSRA decode."""
...
from .litehrnet import LITEHRNET_EXTRAS, LiteHRNet
...
class LiteHRNetPoseConfig:
    variant: str                  # "18" or "30"
    out_channels: int = 17
    input_size: Tuple[int, int] = (192, 256)
    heatmap_size: Tuple[int, int] = (48, 64)
    backbone_out_channels: int = 40   # the highest-res branch width for LiteHRNet-{18,30}
...
LITEHRNET_POSE_COCO_256x192: Dict[str, LiteHRNetPoseConfig] = {
    "18": LiteHRNetPoseConfig(variant="18"),
    "30": LiteHRNetPoseConfig(variant="30"),
}


class LiteHRNetPose(nn.Module):
    def __init__(self, cfg: LiteHRNetPoseConfig) -> None:
        super().__init__()
        self.cfg = cfg
        self.backbone = LiteHRNet(extra=LITEHRNET_EXTRAS[cfg.variant], in_channels=3)
        # HeatmapHead with no deconvs — IterativeHead in LiteHRNet already
        # outputs at stride 4 for the highest-resolution branch.
...
class LiteHRNetPoseInferencer:
    ...
    def from_pretrained(
        ...
        if variant not in LITEHRNET_POSE_COCO_256x192:
            raise KeyError(f"unknown variant {variant!r}")
        cfg = LITEHRNET_POSE_COCO_256x192[variant]
        model = LiteHRNetPose(cfg)
    ...
    @staticmethod
    def _load_checkpoint(model: LiteHRNetPose, path: str | Path) -> None:
        raw = safe_torch_load(path)
        sd = _strip_state_dict(raw)
        missing, unexpected = model.load_state_dict(sd, strict=False)
        # ``num_batches_tracked`` may be missing from old checkpoints — that's
        # harmless, we silently fill with default 0.
        ...
        if unexpected:
            print(f"[FFPose] LiteHRNet: {len(unexpected)} unexpected keys ignored, e.g.: {unexpected[:5]}")
